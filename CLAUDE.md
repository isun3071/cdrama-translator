...
1. **Scope: personal use, no redistribution, in-the-clear video only.** Do not build features that target DRM-protected premium platforms, that share/upload/publish translations, or that export a **standard portable subtitle file** (`.srt`/`.vtt`) — a clean shareable track is the fansub-redistribution artifact this boundary exists to prevent. The direct `<video>` canvas grab returning black on DRM is a *feature* of this boundary, not a bug to defeat. **Personal local re-watch is allowed within these limits** (amended deliberately): the tool may store a corrected translation track and replay it locally for the same user re-watching their own in-the-clear video, provided the track stays on the user's machine, is never shared/uploaded, and is written in the tool's own `frame_id`-keyed replay format — deliberately *not* a portable `.srt`, the same kind of intentional limit as the DRM black screen. **This scope is inherited by any model distilled/fine-tuned from the audit log:** its training targets are translations derived from copyrighted burned-in subtitles, so the student model is personal-use, **not a public deliverable — the extension ships, the model does not.** And its **training data must clear the corpus gate** (≥3 shows / ≥2 genres, a held-out episode): a student distilled from one show's teacher tracks overfits that show's names, register and domain — the same trap the drift/persona gates guard against. The per-show levers (glossary, context note, tone) and a polished per-episode re-watch track are runtime config and personal artifacts, **never evidence the model generalizes.** Keep the wall between per-show runtime config and model training data. Capture this before, not after, distillation work.
